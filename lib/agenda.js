...
function processJobs(extraJob) {
  ...
  /**
   * Number of times we are allowed to recurse before calling function on next event loop
   * @type {number}
   */
  var BREAKUP_STACK_AFTER_RUNS = 100;

  /**
   * Internal method that processes any jobs in the local queue (array)
   * @param {Number} runs number of runs deep into jobProcessing()
   * @returns {undefined}
   */
  function jobProcessing(runs) {

    // Set runs if it does not exist
    if (typeof runs === 'undefined') {
      runs = 0;
    }

    // Break up recursion
    if (runs > BREAKUP_STACK_AFTER_RUNS) {
      setImmediate(jobProcessing.bind(null, 0));
      return;
    }
    ...
    function runOrRetry() {
      if (self._processInterval) {
        if (jobDefinition.concurrency > jobDefinition.running &&
          ...
          job.run(processJobResult.bind(null, runs));
   ...
   * @param {Number} runs how deep into recursion of jobProcessing() we are
   * @param {Error} err thrown if can't process job
   * @param {module.Job} job job to process
   * @returns {undefined}
   */
  function processJobResult(runs, err, job) {
